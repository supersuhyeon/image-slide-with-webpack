export default class ImageSlider{
    ...
    initAutoplay(){
        this.#intervalId = setInterval(()=>{this.moveToRight()},3000) //setinterval은 아이디를 남긴다
        
    }

    initSliderNumber(){ //슬라이드 넘버 초기화, 동적으로 만드니까 슬라이드 몇갠지 파악하고 시작
        this.#sliderNumber = this.sliderListEl.querySelectorAll('li').length //ul>li*7
    }

    initSliderWidth(){ //슬라이드 넓이 초기화
        this.#sliderWidth = this.sliderWrapEl.clientWidth
    }

    initSliderListWidth(){ //ul 전체길이
    ...
    addEvent(){
        ...
        this.indicatorWrapEl.addEventListener('click', (e)=>{ //이벤트위임
    ...
    onClickIndicator(e){
        ...
        if(Number.isInteger(indexPosition)){ //정수면 실행시킬것
    ...
    moveToRight(){
        ...
        if(this.#currentPosition === this.#sliderNumber){ //경계값처리
        ...
        if(this.#autoPlay){
            clearInterval(this.#intervalId)
            this.#intervalId = setInterval(()=>{this.moveToRight()},3000) //버튼을 눌러도 삼초뒤에 지나가도록
        }
        this.setIndicator()
    }
    moveToLeft(){
    ...
    setIndicator(){ //해당슬라이더 넘버에 맞는 인디케이터 컬러 활성화시키기
       this.indicatorWrapEl.querySelector('li.active')?.classList.remove('active')
       this.indicatorWrapEl.querySelector(`ul li:nth-child(${this.#currentPosition + 1})`).classList.add('active') //nthchild는 1부터시작
    }
}